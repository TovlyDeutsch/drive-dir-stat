...
function renderDirStructure(file, level) {
  let children = [];
  if (
    file.hasOwnProperty("children") &&
    Object.entries(file.children).length > 0
  ) {
    let sortedChildren = Object.values(file.children).sort(
      (file1, file2) => file2.bytes - file1.bytes
    );
